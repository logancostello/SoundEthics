...
def generate_with_ace(audio_path, prompt, bpm, duration, inference_steps, seed, key, similarity):
    payload = {
        "caption": prompt,
        ...
        "guidance_scale": similarity, # todo: make sure this is correct
    }
...
def upload_file():
    if request.method == 'POST':
        ...
        similarity = request.form.get("similarity") or -1
        ...
        for stem_type, stem_filepath in split_filepaths.items():
            ...
            ace_output = generate_with_ace(stem_filepath, prompt, bpm, duration, inference_steps, seed, key, similarity)
